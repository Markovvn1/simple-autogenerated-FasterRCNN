...
def build_backbone():
	...
	with open("build/backbone.py", "w") as f:
		f.write(generate_backbone(with_include=True))


def generate_backbone(with_include):
	assert isinstance(with_include, bool)

	n2 = "\n\n"

	return f"""\
{"import torch.nn as nn"+n2 if with_include else ""}\
