...
class ModuleBuilder(ModuleBuilderBase):
	...
	def __init__(self):
		super().__init__({"freeze_batchnorm.ModuleWithFreeze", "freeze_batchnorm.FrozenBatchNorm2d"})

	def _assert_cfg(self, module, cfg):
		assert not cfg

	def _dependencies(self, module, global_params, cfg):
		if module == "freeze_batchnorm.ModuleWithFreeze":
			return {"freeze_batchnorm.FrozenBatchNorm2d": None}
		return {}
...
def _generate(global_params, dep):
	...
	if "freeze_batchnorm.ModuleWithFreeze" in dep:
		res.append("""\n
class ModuleWithFreeze:
	...
	if "freeze_batchnorm.FrozenBatchNorm2d" in dep:
		res.append("""\n
